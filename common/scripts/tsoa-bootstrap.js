...
function getTsoaCommand(type) {
  ...
  if (!validTypes.includes(type)) {
    throw new Error(
      `Invalid TSOA command type: ${type}. Must be one of: ${validTypes.join(", ")}`,
    );
  }
...
function temporarilyPatchControllerImports() {
  try {
    ...
    const controllers = fs
      ...
      .filter((file) => file.endsWith(".ts") && !file.startsWith("tsoa-"));
...
function restoreControllerImports(backupDir) {
  try {
    ...
    if (!SKIP_CLEANUP) {
      ...
      console.log(
        "Skipping temporary file cleanup due to DEBUG_MODE being enabled",
      );
    }
...
function checkTsoaConfiguration() {
  ...
  try {
    ...
    if (
      !tsoaConfig.modelsPathGlobs ||
      !Array.isArray(tsoaConfig.modelsPathGlobs) ||
      ...
      if (
        !tsoaConfig.modelsPathGlobs.includes(
          "./src/types/api/tsoa-reference.ts",
        )
      ) {
      ...
      console.log(
        "Updated tsoa.json with improved modelsPathGlobs configuration",
      );
    }
...
function updateTsoaTsConfig() {
  ...
  try {
    ...
    tsConfig.compilerOptions.paths["common/schema/types"] = [
      "./src/schema/types.ts",
    ];
    tsConfig.compilerOptions.paths["common/src/schema/types"] = [
      "./src/schema/types.ts",
    ];
    tsConfig.compilerOptions.paths["common/src/types/api/*"] = [
      "./src/types/api/*",
    ];
