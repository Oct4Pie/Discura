...
export class KnowledgeRepository extends BaseRepository<KnowledgeItemEntity> {
  ...
  async findByBotId(botId: string): Promise<KnowledgeItemEntity[]> {
    try {
      ...
      logger.error(
        `Error fetching knowledge items for bot ID ${botId}:`,
        error,
      );
      throw error;
    }
  ...
  async createKnowledgeItem(
    item: Omit<KnowledgeItemEntity, "id" | "created_at" | "updated_at">,
  ...
  async updateKnowledgeItem(
    ...
    try {
      ...
      return (await db.update(this.tableName, updatedItem, "id = ?", [id])) > 0;
    } catch (error) {
  ...
  async deleteAllForBot(botId: string): Promise<boolean> {
    try {
      ...
      logger.error(
        `Error deleting all knowledge items for bot ${botId}:`,
        error,
      );
      throw error;
    }
