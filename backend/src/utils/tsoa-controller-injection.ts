...
export function injectBackendControllers(): void {
  try {
    ...
    Object.values(backendControllers).forEach((ControllerClass: any) => {
      ...
      if (typeof ControllerClass === 'function' && ControllerClass.name.endsWith('Controller')) {
        ...
        if (commonControllerClass) {
          logger.info(`Found backend implementation for: ${ControllerClass.name}`);
          ...
          const protoMethods = Object.getOwnPropertyNames(ControllerClass.prototype)
            .filter(name => name !== 'constructor' && typeof backendInstance[name] === 'function');
          ...
          protoMethods.forEach(methodName => {
            ...
            commonControllerClass.prototype[methodName] = backendInstance[methodName].bind(backendInstance);
          });
          
          logger.info(`Successfully injected backend implementation for ${ControllerClass.name}`);
        } else {
          // This might happen if a backend controller doesn't have a corresponding common controller
          logger.warn(`No common controller found for backend controller: ${ControllerClass.name}`);
        }
      }
    });
    ...
    Object.values(commonControllers).forEach((CommonControllerClass: any) => {
      if (typeof CommonControllerClass === 'function' && CommonControllerClass.name.endsWith('Controller')) {
        // Find our corresponding backend controller instance
        const backendInstance = controllerInstances.get(CommonControllerClass.name);
        
        if (backendInstance) {
          // Store the original constructor to restore it later
          const originalConstructor = CommonControllerClass.prototype.constructor;
    ...
    logger.info(`Successfully injected backend controller implementations via route proxy`);
  } catch (error) {
    ...
    throw new Error(`Controller injection failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export function restoreOriginalControllers(): void {
  try {
    ...
    Object.values(commonControllers).forEach((CommonControllerClass: any) => {
      if (typeof CommonControllerClass === 'function' && 
          CommonControllerClass.name.endsWith('Controller') &&
          CommonControllerClass._originalConstructor) {
        
        // Restore original constructor
        CommonControllerClass.prototype.constructor = CommonControllerClass._originalConstructor;
